...
function buildModelFMU([string]$modelname = $(Mandatory=$true), [string]$shortmodelname = $(Mandatory=$true)){
    Add-Content $scriptfile -value('buildResult:=buildModelFMU('+$modelname+', version="2.0", fileNamePrefix="'+$shortmodelname+'", fmuType="cs", platforms={"static"}, includeResources=true);')
    # Add-Content $scriptfile 'clearMessages();'
    Add-Content $scriptfile 'errorstr:=getErrorString(false);'
    Add-Content $scriptfile 'buildSuccess:=regularFileExists(buildResult[1]+".exe");'
    Add-Content $scriptfile 'exitval:= if buildSuccess then 0 else 1;'
    Add-Content $scriptfile -value('writeFileReturn:= if errorstr=="" then false else writeFile("../'+$modelname+'_errlog.txt", errorstr, false);')
...
function buildModel([string]$modelname = $(Mandatory=$true), [string]$shortmodelname = $(Mandatory=$true)){
    Add-Content $scriptfile -value('buildResult:=buildModel('+$modelname+', fileNamePrefix="'+$shortmodelname+'", startTime=0, stopTime=1, numberOfIntervals=1000, tolerance=1e-5, method="dassl", variableFilter=".*", cflags="", outputFormat="mat", simflags="-lv=LOG_STATS,LOG_SIMULATION");')
    # Add-Content $scriptfile 'clearMessages();'
    Add-Content $scriptfile 'errorstr:=getErrorString(false);'
    Add-Content $scriptfile 'buildSuccess:=regularFileExists(buildResult[1]+".exe");'
    Add-Content $scriptfile 'exitval:= if buildSuccess then 0 else 1;'
...
# compilation: suppose that 'inp' contains the model path or the model name relative to CrtVehicle
...
if(Test-Path $inp){ # build without including CrtVehicle
    ...
    if($fmu){ #noAltair FMU
        Write-Host -NoNewLine "Building $modelname into an FMU: "
        ...
        Write-Host -NoNewLine "Building $modelname into an executable: "
    ...
    if ($CrtVehicle_found) {
        ...
        if($shortmodelname_found){
            ...
            if($fmu){ #Altair FMU
                Write-Host -NoNewLine "Building CrtVehicle model $modelname into an FMU: "
                $workfolder = "_fmu_temp\$shortmodelname\"
                prepareBuildEnv -workfolder $workfolder
                ...
                Write-Host -NoNewLine "Building CrtVehicle model $modelname into an executable: "
                $workfolder = "_build_temp\$shortmodelname\"
                prepareBuildEnv -workfolder $workfolder
                loadAltairLibrary -scriptfile $scriptfile
                buildModel -modelname $modelname -shortmodelname $shortmodelname
...
if ($omcerrorflag){
    Write-Host "FAILED with error code $omcerrorflag"
}
else{
    Write-Host "SUCCESS"
    if(!$donotnotify){
